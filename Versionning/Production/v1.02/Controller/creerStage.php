...
else
{
    if(isset($_POST['fonctionTuteur']))
    {
        ...
        if(isset($_POST['tuteurIdentique']))
        {
            if(isset($_POST['signataireExistant']))
            {
                $signataireExistant = getInfosSignataire((int)$_POST['choixSignataireExistant'], $connexion);
                $nom = $signataireExistant->nom;
                $prenom = $signataireExistant->prenom;
                $telephone = $signataireExistant->telephone;
                $email = $signataireExistant->email;
                $fonction = $signataireExistant->fonction;
                $codeOrganisme = $signataireExistant->codeOrganisme;
                $tuteurExistant = getCodeTuteur($nom, $prenom, $telephone, $email, $fonction, $codeOrganisme, $connexion)->codeEmploye;
                if($tuteurExistant == false)
                {
                    $codeTuteur = (int)createEmploye($nom, $prenom, $telephone, $email, $fonction, $codeOrganisme, 2, $connexion);
                    createTuteur($codeTuteur, $connexion);
                }
                else
                {
                    $codeTuteur = (int)$tuteurExistant;
                }
            }
            else
            {
            ...
            <body style="color: #888; font-family: \'Helvetica Neue\',Helvetica,Arial,sans-serif; font-size: 14px; line-height: 1.42857; background-color: #FFF; margin: 0px;">
                <div>
                    <div style="text-align: center; margin: 30px auto auto; min-width: 290px; width: 25%; box-shadow: 0px 1px 2px rgba(0, 0, 0, 0.05); background-color: #FFF; border: 1px solid #DDD; border-radius: 4px;">
                        <div style="height: 240px; background: transparent url('.$lienAccueil.'/Authentification/Base/Ressources/Images/logoEasyStage.png) no-repeat scroll center top; border: medium none; color: #333; padding: 10px 15px; border-top-left-radius: 3px; border-top-right-radius: 3px; text-align: center;"> 
                        </div>
                        <div style="padding: 15px; text-align: center;">
                            ...
                            <a href="'.$lienAccueil.'" style="text-decoration: none; cursor: pointer; background-color: #466FC1; color: #FFF; display: inline-block; padding: 6px 12px; margin-bottom: 0px; font-size: 14px; font-weight: 400; line-height: 1.42857; text-align: center; white-space: nowrap; vertical-align: middle; -moz-user-select: none; background-image: none; border: 1px solid transparent; border-radius: 4px;">
